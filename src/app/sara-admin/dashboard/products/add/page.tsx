    ...
    try {
      ...
      if (response.ok && data.specifications) {
        ...
        setSpecifications(prev => {
           // Filter out NEW specs that match existing titles
  ...
  const handleGenerateAIVariations = async () => {
    ...
    try {
      ...
      if (response.ok && data.variations) {
        ...
        setVariations(prev => {
          // Filter out NEW variations that match existing titles (case-insensitive)
          // "If variations contain already size... don't change that"
          const existingTitles = new Set(prev.map(p => p.title.toLowerCase()))
