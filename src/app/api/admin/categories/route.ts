...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Check for permanent categories
    const permanentCategories = await Category.find({ _id: { $in: categoryIds }, isPermanent: true })
    if (permanentCategories.length > 0) {
      return NextResponse.json({ success: false, error: 'Cannot delete permanent categories' }, { status: 400 })
    }
    
    await Category.deleteMany({ _id: { $in: categoryIds }, isPermanent: { $ne: true } })
