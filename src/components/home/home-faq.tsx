          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                ...
                value={`item-${index}`}
                className="border border-[#d4af37]/20 rounded-2xl bg-white dark:bg-[#1e1e1e] overflow-hidden group hover:shadow-lg transition-all duration-300"
              >
                <AccordionTrigger className="text-left px-6 py-5 text-lg font-serif font-medium text-gray-900 dark:text-white hover:text-[#d4af37] dark:hover:text-[#d4af37] transition-all duration-300 [&[data-state=open]]:text-[#d4af37]">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="text-gray-600 dark:text-gray-300 bg-gradient-to-br from-[#faf8f3] to-white dark:from-[#2a2a2a] dark:to-[#1e1e1e] px-6 py-5 leading-relaxed border-t border-[#d4af37]/10">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
