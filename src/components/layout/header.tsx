        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <div ref={logoRef} className="flex items-center md:flex-none flex-1 justify-center md:justify-start">
              <Link href="/" className="flex items-center gap-3">
                ...
                <div className="flex flex-col items-center">
                  <span className="text-xl font-serif italic font-bold text-[#d4af37] dark:text-[#d4af37] tracking-wider" style={{ fontFamily: 'var(--font-serif)' }}>SARA</span>
                  <span className="text-xl font-serif italic font-bold text-[#d4af37] dark:text-[#d4af37] -mt-1 tracking-wider" style={{ fontFamily: 'var(--font-serif)' }}>JEWELERS</span>
                </div>
              </Link>
            </div>
            ...
            <div ref={iconsRef} className="flex items-center gap-2 md:gap-4">
              ...
              {/* User Account with Premium Dropdown */}
              {session ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="cursor-pointer outline-none">
                      <Avatar className="size-8 hover:ring-2 hover:ring-[#d4af37]/50 transition-all">
                  ...
                  <DropdownMenuContent className="w-64 mt-2 bg-white/95 dark:bg-[#1e1e1e]/95 backdrop-blur-xl border border-[#d4af37]/20 shadow-2xl rounded-2xl p-2" align="end">
                    <div className="px-3 py-3 border-b border-gray-100 dark:border-gray-800">
                      <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{session.user?.name}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{session.user?.email}</p>
                    </div>
                    <DropdownMenuItem asChild className="cursor-pointer rounded-xl my-1">
                      <Link href="/account" className="flex items-center gap-3 px-3 py-2.5 hover:bg-[#f8f4e8] dark:hover:bg-[#2a2a2a] transition-colors">
                        <User size={18} className="text-[#d4af37]" />
                        <span className="text-sm font-medium">My Profile</span>
                      </Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild className="cursor-pointer rounded-xl my-1">
                      <Link href="/account?tab=orders" className="flex items-center gap-3 px-3 py-2.5 hover:bg-[#f8f4e8] dark:hover:bg-[#2a2a2a] transition-colors">
                        <Package size={18} className="text-[#d4af37]" />
                        <span className="text-sm font-medium">Order History</span>
                      </Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild className="cursor-pointer rounded-xl my-1">
                      <Link href="/wishlist" className="flex items-center gap-3 px-3 py-2.5 hover:bg-[#f8f4e8] dark:hover:bg-[#2a2a2a] transition-colors">
                    ...
                    <DropdownMenuItem 
                      ...
                      className="cursor-pointer rounded-xl px-3 py-2.5 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                    >
              ...
              <Link href="/account">
                <User
                  size={22}
                  className="text-[#555555] hover:text-[#d4af37] dark:text-[#cccccc] dark:hover:text-[#d4af37] transition-colors cursor-pointer"
                />
              </Link>
              )}
            </div>
                    ...
                    <div className="flex-1 flex gap-8">
                      {navLinks.find(link => link.name === openDropdown)?.dropdown?.map((col: any, index: number) => (
                        <div key={index} className="flex-1">
                          <Link 
                            href={col.href || "#"} 
                            className="block text-lg font-serif italic font-medium text-[#d4af37] hover:text-[#f4d03f] pb-2 border-b border-[#d4af37]/20 mb-3 transition-colors tracking-wide"
              ...
              <nav className="flex flex-col space-y-4">
                ...
                {session ? (
                  <Link
                    href="/account"
                    className={`font-medium transition-all duration-200 flex items-center p-3 rounded-lg menu-item-animate hover:bg-[#f8f4e8] dark:hover:bg-[#303846] hover:scale-105 ${
                      pathname === "/account" ? "bg-[#f8f4e8] text-[#d4af37] dark:bg-[#303846] shadow-sm" : "text-[#555555] dark:text-[#cccccc]"
                    }`}
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    <Avatar className="size-6 me-2 ring-1 ring-[#d4af37]/30">
                      <AvatarImage src={session.user?.image || undefined} alt="User" />
                      <AvatarFallback className="bg-[#d4af37] text-white text-[10px] font-bold">
                        {session.user?.name?.charAt(0) || 'U'}
                      </AvatarFallback>
                    </Avatar>
                    My Profile
                  ...
                  <Link
                    href="/account"
                    className="w-full justify-start hover:bg-[#f8f4e8] dark:hover:bg-[#303846] transition-all duration-200 border-[#d4af37] text-[#d4af37] menu-item-animate hover:scale-105 p-3 rounded-lg border flex items-center"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
